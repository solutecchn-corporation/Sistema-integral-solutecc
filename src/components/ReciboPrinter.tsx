...
export async function printMovimientoReceipt(m: MovimientoForPrint) {
  try {
    ...
    const companyEmail =
      company?.correo || company?.email || company?.mail || "";
...
export default function ReciboPrinterPreview({
  movimiento,
}: {
  movimiento: MovimientoForPrint;
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ marginTop: 8 }}>
        <strong>Total:</strong> L{formatMoney(movimiento.monto)}
      </div>
