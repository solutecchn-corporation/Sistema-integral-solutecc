...
export default function PedidosEnLinea({ onBack }: { onBack: () => void }) {
  ...
  const fetchPedidos = useCallback(async () => {
    ...
    try {
      let q = (supabase as any)
        .from("pedidos_web")
        .select(
          "id, pedido_numero, fecha_pedido, subtotal, impuesto, total, estado, usuario_id",
        )
  ...
  const loadDetalle = async (pedidoId: string) => {
    ...
    try {
      const { data, error: err } = await (supabase as any)
        ...
        .select(
          "id, pedido_id, producto_id, cantidad, precio_unitario, subtotal, descuento, total",
        )
        .eq("pedido_id", pedidoId);
      if (err) throw err;
      let rows: DetallePedido[] = Array.isArray(data)
        ? (data as DetallePedido[])
        : [];

      // Enrich con nombres del inventario
      const prodIds = Array.from(
        new Set(rows.map((d) => d.producto_id).filter(Boolean)),
      );
      if (prodIds.length > 0) {
        try {
          ...
          if (Array.isArray(prods)) {
            ...
            rows = rows.map((d) => ({
              ...d,
              nombre: pm[d.producto_id]?.nombre ?? null,
              sku: pm[d.producto_id]?.sku ?? null,
            }));
          }
        } catch (e) {
          console.warn("Error enriching detalles:", e);
        }
      }
  ...
  const fmtDate = (s: string | null) => {
    if (!s) return "-";
    try {
      return new Date(s).toLocaleString("es-HN", {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
        hour: "2-digit",
    ...
    >
      ...
      {!loading && pedidos.length === 0 && (
        ...
        >
          No hay pedidos{" "}
          {filtroEstado !== "todos"
            ? `con estado "${ESTADO_BADGE[filtroEstado]?.label ?? filtroEstado}"`
            : ""}
          .
        </div>
      )}
        ...
        {pedidos.map((pedido) => {
          ...
          return (
            ...
            >
              ...
              >
                <div style={{ fontWeight: 700, fontSize: 15, minWidth: 90 }}>
                ...
                <div style={{ fontSize: 12, color: "#6b7280", minWidth: 130 }}>
        ...
        >
          ...
          >
            ...
            <div style={{ color: "#6b7280", fontSize: 13, marginBottom: 20 }}>
              El pedido se marcará como <strong>Cancelado</strong>. Esta acción
